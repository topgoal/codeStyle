...
class VerificationLoginVC: BaseViewController {
    ...
    func initViewModel() {
        // 初始化视图模型
        let viewModel = VerificationLoginViewModel(input: (
            ...
            verificationLoginTap: confirmButton.rx.tap.asDriver()))
        ...
        // 是否可登陆
        viewModel.verificationLoginEnabled
            .drive(onNext: { [weak self] enable in
        ...
        // 注册状态
        let verificationLoginProgress = MBProgressHUD.createHUD(view: self.view)
        viewModel.verificationLoginLoading
            .drive(verificationLoginProgress.rx.isAnimating)
            .disposed(by: disposeBag)
        
        // 注册请求结果
        viewModel.verificationLoginPostResult.asDriver().drive(onNext: { [weak self] postResult in
            switch postResult {
            case .ok(let message):
                self?.toast(message)
